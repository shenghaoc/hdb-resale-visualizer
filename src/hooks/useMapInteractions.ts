...
export function useMapInteractions({
  ...
  useEffect(() => {
    ...
    const handleClickUnclustered = (event: any) => {
    ...
    const handleClickCluster = (event: any) => {
      onMapInteractRef.current?.("feature");
      const feature = event.features?.[0];
    ...
    const handleMouseEnterUnclustered = (event: any) => {
      ...
      addressEl.textContent = props.address ?? "";
      container.appendChild(addressEl);

      if (props.display_name) {
        const nameEl = document.createElement("p");
        nameEl.textContent = props.display_name;
        container.appendChild(nameEl);
      }

      const townEl = document.createElement("p");
      townEl.textContent = localizeTownName(props.town ?? "", localeRef.current);
      container.appendChild(townEl);

      const infoEl = document.createElement("p");
      infoEl.textContent = `${tRef.current("map.median", { value: formatCompactCurrency(props.median_price ?? 0) })} · ${tRef.current("map.txns", { count: props.transaction_count ?? 0 })}`;
