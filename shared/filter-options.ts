...
export function sortFlatTypes(flatTypes: string[]): string[] {
  const order = ["1 ROOM", "2 ROOM", "3 ROOM", "4 ROOM", "5 ROOM", "EXECUTIVE", "MULTI-GENERATION"];
...
export function buildFilterOptions(
  ...
  for (const block of blocks) {
    ...
    for (const flatType of block.flatTypes) flatTypes.add(canonicalFlatType(flatType));
