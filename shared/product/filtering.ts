/**
 * Platform-neutral filtering and geographic search logic.
 *
 * Provides the core filter predicate (`matchesFilter`), geographic search
 * intent resolution, station name matching, coordinate/radius filtering,
 * and effective median price helpers.
 *
 * All caches are module-level and resettable for deterministic tests via
...
export function matchesGeographicSearchIntent(
  ...
  if (intent.type === "coordinates") {
    // Fast-path bounding box check before expensive haversine math.
...
export function createFilterEvaluationContext(currentYear: number): FilterEvaluationContext {
  return { currentYear };
}
...
export function matchesFilter(
  ...
  // caching layer) and passed in as a boolean. This keeps the shared predicate
  // free of framework-specific caching dependencies.
  if (filters.affordable && passesAffordability === false) {
    return false;
  }
