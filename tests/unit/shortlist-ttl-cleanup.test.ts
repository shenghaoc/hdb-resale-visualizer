...
describe("shortlistRetentionCutoff", () => {
  it("subtracts SHORTLIST_RETENTION_DAYS from the supplied clock", () => {
    ...
    const cutoff = shortlistRetentionCutoff(now.getTime());
    const expected = new Date(now.getTime() - SHORTLIST_RETENTION_MS);
...
describe("purgeStaleShortlists", () => {
  it("deletes only rows older than the retention window, batched with LIMIT", async () => {
    ...
    const now = new Date("2026-05-27T12:00:00.000Z");
    const deleted = await purgeStaleShortlists(db, now.getTime());
    ...
    expect(deleteCalls[0]?.args).toEqual([shortlistRetentionCutoff(now.getTime()), 1000]);
  });
  ...
  it("loops until fewer than pageSize rows are deleted", async () => {
    ...
    expect(deleteCalls[0]?.args).toEqual([shortlistRetentionCutoff(now.getTime()), 1000]);
    expect(deleteCalls[2]?.args).toEqual([shortlistRetentionCutoff(now.getTime()), 1000]);
  });
});
