...
**Learning:** While  ensures screen reader accessibility, icon-only buttons remain ambiguous for sighted mouse users. Adding a native  attribute that matches the  provides a lightweight, zero-dependency tooltip.
**Action:** Always verify that icon-only interactive elements possess both an  and a  attribute.
