...
- [ ] **T1.1** Keep comparable selection in
...
- [ ] **T1.2** Keep request construction, canonical storey options,
...
- [ ] **T1.3** Use `shared/confidence-system.ts` and
...
- [ ] **T2.1** Remove the user-facing legacy detail-drawer evaluator and any
...
- [ ] **T2.2** Keep distribution and comparable-evidence presentation
...
- [ ] **T3.1** Implement `src/features/listing-check/ListingCheckPanel.tsx`
...
- [ ] **T3.4** Keep the empty state free of a sample CTA and synthetic unit
...
- [ ] **T6.6** Route the block-detail "Check this listing" action to the
...
- [ ] **T7.1** Cover canonical confidence signals and caps in the shared
...
- [ ] **T7.3** Extend `tests/unit/listingCheckAnalysis.test.ts` and
...
- [ ] **T8.1** Extend `tests/components/ListingCheckPanel.inputs.test.tsx`:
...
- [ ] **T8.2** Extend `tests/e2e/buyer-listing-check.spec.ts`: mobile and
...
- [ ] **T8.3** Add block-detail regression coverage proving its Check action
