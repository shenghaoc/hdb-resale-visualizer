...
## R2: Generated Artifact Contract
Generated artifacts are release-critical product inputs and must be treated as pipeline-owned outputs.

1. Runtime UI features may consume only artifact fields that are deterministically generated by `scripts/sync-data.ts`.
2. Generated files under `public/data/` must not be manually edited; updates must come from the pipeline.
3. UI/typing changes that depend on artifact shape updates must keep `scripts/lib/schemas.ts` and `src/types/data.ts` synchronized.
4. Features that need new comparison data are blocked until the corresponding artifact exists and is validated by tests.
