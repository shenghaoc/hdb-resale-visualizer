...
## Phase 9: Global UI Redesign
- [x] Integrate modern, minimalist design tokens generated by v0 AI into `styles.css`.
- [x] Replace Noto Sans with Inter for typography layout.
- [x] Implement the `panel-glass` overlay style to `Card` and `Drawer` components to allow high map visibility in App shell overlay.
- [x] Fix exhaustive hooks dependency linting errors in App shell.
